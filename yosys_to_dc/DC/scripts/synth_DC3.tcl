...
set fldr combined_shifted
# .v files, separated by spaces
set my_verilog_files [list $fldr/BDI_B1D0_S.v $fldr/BDI_B2D1_S.v $fldr/BDI_B4D2D1_S.v $fldr/BDI_B8D0_S.v $fldr/BDI_B8D4D2D1_S.v $fldr/BDI_TOP_S.v $fldr/BDI_TOP_S_FF.v ]
#/* Top-level Module                               	*/
set my_toplevel BDI_TOP_S_FF
...
# Dump output files
#------------------------------------------------------------------------
set filename [format "%s%s"  $fldr/$my_toplevel ".vh"]
write -f verilog -output $filename

set sdcOutFile [format "%s%s"  $fldr/$my_toplevel ".sdc"]
write_sdc $sdcOutFile

# amin eddit
#set filename [format "%s%s"  $fldr/$my_toplevel ".db"]
